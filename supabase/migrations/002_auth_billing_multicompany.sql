...
-- ── Enums (idempotent for partial / rerun) ─────────────────
DO $$ BEGIN CREATE TYPE user_role AS ENUM ('owner', 'accountant', 'bookkeeper', 'read_only'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE plan_name AS ENUM ('starter', 'pro', 'accountant', 'firm'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE billing_model AS ENUM ('client_pays', 'accountant_pays', 'revenue_share'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN CREATE TYPE reconciliation_status AS ENUM ('in_progress', 'completed', 'locked'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE categorization_status AS ENUM ('unreviewed', 'ai_suggested', 'confirmed', 'overridden'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE TYPE plaid_status AS ENUM ('active', 'error', 'pending_expiration', 'revoked'); EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE TABLE IF NOT EXISTS ai_categorization_signals (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  org_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_ai_signals_merchant ON ai_categorization_signals(org_id, merchant_name);
CREATE INDEX IF NOT EXISTS idx_ai_signals_description ON ai_categorization_signals(org_id, description_normalized);
...
CREATE INDEX IF NOT EXISTS idx_bank_tx_categorization ON bank_transactions(org_id, categorization_status);
...
DROP TRIGGER IF EXISTS trg_check_period_lock ON bank_transactions;
...
DROP TRIGGER IF EXISTS set_plaid_connections_updated_at ON plaid_connections;
DROP TRIGGER IF EXISTS set_reconciliation_periods_updated_at ON reconciliation_periods;
DROP TRIGGER IF EXISTS set_mfa_settings_updated_at ON mfa_settings;
...
DROP POLICY IF EXISTS "memberships_org" ON company_memberships;
...
DROP POLICY IF EXISTS "plaid_org" ON plaid_connections;
...
DROP POLICY IF EXISTS "ai_signals_org" ON ai_categorization_signals;
...
DROP POLICY IF EXISTS "reconciliation_org" ON reconciliation_periods;
...
DROP POLICY IF EXISTS "csv_imports_org" ON csv_import_jobs;
CREATE POLICY "csv_imports_org" ON csv_import_jobs FOR ALL USING (org_id = current_org_id());

ALTER TABLE firms ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "firms_member" ON firms;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'invitations'
  ) THEN
