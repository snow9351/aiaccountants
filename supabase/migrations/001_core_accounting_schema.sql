...
DROP TRIGGER IF EXISTS trg_tax_settings_updated_at ON tax_settings;
...
DROP POLICY IF EXISTS "org_all_customers" ON customers;
DROP POLICY IF EXISTS "org_all_invoices" ON invoices;
DROP POLICY IF EXISTS "org_all_inv_lines" ON invoice_line_items;
DROP POLICY IF EXISTS "org_all_inv_pmts" ON invoice_payments;
DROP POLICY IF EXISTS "org_all_vendors" ON vendors;
DROP POLICY IF EXISTS "org_all_bills" ON bills;
DROP POLICY IF EXISTS "org_all_bill_lines" ON bill_line_items;
DROP POLICY IF EXISTS "org_all_bill_pmts" ON bill_payments;
DROP POLICY IF EXISTS "org_all_expenses" ON expenses;
DROP POLICY IF EXISTS "org_all_receipts" ON receipts;
DROP POLICY IF EXISTS "org_all_accounts" ON accounts;
DROP POLICY IF EXISTS "org_all_je" ON journal_entries;
DROP POLICY IF EXISTS "org_all_je_lines" ON journal_entry_lines;
DROP POLICY IF EXISTS "org_all_bank_accts" ON bank_accounts;
DROP POLICY IF EXISTS "org_all_bank_txns" ON bank_transactions;
DROP POLICY IF EXISTS "org_all_recon" ON bank_reconciliations;
DROP POLICY IF EXISTS "org_all_employees" ON employees;
DROP POLICY IF EXISTS "org_all_timesheets" ON timesheets;
DROP POLICY IF EXISTS "org_all_ts_entries" ON timesheet_entries;
DROP POLICY IF EXISTS "org_all_payroll_run" ON payroll_runs;
DROP POLICY IF EXISTS "org_all_payroll_dtl" ON payroll_details;
DROP POLICY IF EXISTS "org_all_budgets" ON budgets;
DROP POLICY IF EXISTS "org_all_budget_lines" ON budget_lines;
DROP POLICY IF EXISTS "org_all_projects" ON projects;
DROP POLICY IF EXISTS "org_all_proj_phases" ON project_phases;
DROP POLICY IF EXISTS "org_all_ai_alerts" ON ai_alerts;
DROP POLICY IF EXISTS "org_all_anomalies" ON anomalies_detected;
DROP POLICY IF EXISTS "org_all_audit" ON audit_log;
DROP POLICY IF EXISTS "audit_insert" ON audit_log;
DROP POLICY IF EXISTS "org_all_tax_settings" ON tax_settings;
DROP POLICY IF EXISTS "org_all_est_tax" ON estimated_tax_payments;
DROP POLICY IF EXISTS "org_all_payment_terms" ON payment_terms;
DROP POLICY IF EXISTS "org_all_exchanges" ON exchange_rates;
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime' AND tablename = 'ai_alerts'
  ) THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE ai_alerts;
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables
    WHERE pubname = 'supabase_realtime' AND tablename = 'bank_transactions'
  ) THEN
...
DROP TRIGGER IF EXISTS trg_seed_new_org ON organizations;
